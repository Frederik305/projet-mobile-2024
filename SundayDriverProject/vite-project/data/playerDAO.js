import Player from '../model/Player.js';
class playerDAO{
    constructor(){
        this.player = {};
        this.defaultplayer={id: 0, username: 'Player1',highscore: 0,picture:  'data:image/jpeg;base64,', level: 1, cash: 10000, carsUnlocked: [0,1,4]};
    }

    getPlayer(){
        
        if(localStorage['player']){
            this.player = JSON.parse(localStorage['player']);
        }
        else{
            
            localStorage['player']=JSON.stringify(this.defaultplayer);
            this.player = JSON.parse(localStorage['player']);
        }
        let player= new Player(this.player.id, this.player.username, this.player.highscore, this.player.picture, this.player.level, this.player.cash, this.player.carsUnlocked)
        this.player=player

        console.log(this.player.level);
        
        return this.player;
    }

    modifierInfoPlayer(player) {
        
        if (this.player) {
            this.player.username=player.username;
            this.player.picture=player.picture;
            this.player.highscore=player.highscore;
            this.player.level=player.level;
            this.player.cash=player.cash;
            this.player.carsUnlocked=player.carsUnlocked;

            localStorage['player'] = JSON.stringify(this.player);
        } else {
            console.error("Le player avec l'ID spécifié n'existe pas.");
        }
    }

    modifierHighscore(score){
        if (this.player) {
            this.player.highscore=score;
            localStorage['player'] = JSON.stringify(this.player);}
    }

    modifierLevel(experience) {
        if (this.player) {
            experience = parseInt(experience);

            experience/=((this.player.level+10) * 15745);

            this.player.level += experience;
            localStorage['player'] = JSON.stringify(this.player);}
    }

    isRegistered(){
        if(localStorage['player']){
            this.player = JSON.parse(localStorage['player']);
            return true;
        }else{return false;}
    }

    
}
export default playerDAO;